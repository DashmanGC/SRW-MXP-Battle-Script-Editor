...
public class UserInterfaceBSE extends javax.swing.JFrame {
    ArrayList < BattleLine > blines;   // Non-repeated lines
    ...
    String current_file = "";
    String title = "SRW GC Battle Script Editor v1.0a by Dashman";
    ...
    int off_table;  // Offset of the pointer table in the file
    ...
    private void initComponents() {
        ...
        setTitle("SRW MXP Battle Script Editor v1.0c by Dashman");
        setResizable(false);
    ...
    private void menuitemOpenActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_menuitemOpenActionPerformed
        ...
        chooser.setDialogTitle("Load BATTLE file");
        chooser.setFileFilter(new FileNameExtensionFilter("BATTLE file", "BATTLE"));
    ...
    private void menuitemSaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_menuitemSaveActionPerformed
        ...
        if (file_loaded){
            JFileChooser chooser = new JFileChooser();
            chooser.setCurrentDirectory(new java.io.File(lastDirectory));
            chooser.setDialogTitle("Save BATTLE file");
            chooser.setFileFilter(new FileNameExtensionFilter("BATTLE file", "BATTLE"));
            if (!current_file.isEmpty())
                chooser.setSelectedFile(new File(current_file));
    ...
    public void parseFile(String path){
        ...
        try {
            ...
            off_table = (aux[0] & 0xff) + ( (aux[1] & 0xff) << 8 );
            int off_voices = (aux[2] & 0xff) + ( (aux[3] & 0xff) << 8 );
            int off_text = (aux[4] & 0xff) + ( (aux[5] & 0xff) << 8 );

            int num_entries = (off_text - off_voices) / 4;
            
            //System.out.println("OFF_voices: " + off_voices + " - OFF_text: " + off_text);
            ...
            if (num_entries == 0){
                ...
                JOptionPane.showMessageDialog(null, "Number of battle lines in the BATTLE file: 0",
                    "Error", JOptionPane.ERROR_MESSAGE);
            ...
            first_section = new byte[off_text];
            
            f.read(first_section);
            
            // Get the pointer table
            int table_size = num_entries * 2;
                        
            byte[] table = new byte[table_size];
            
            for (int i = 0; i < table_size; i++){
                table[i] = first_section[off_table + i];
            }
            ...
            for (int i = 0; i < num_entries; i++){
                start = (table[i*2] & 0xff) + ( (table[(i*2) + 1] & 0xff) << 8 );
                
                //System.out.println("Start: " + start);                
                
                if (i < num_entries - 1){
                    end = (table[(i*2) + 2] & 0xff) + ( (table[(i*2) + 3] & 0xff) << 8 );
                    
                    end--;  // We don't need the 00 byte
                    ...
                    do{
                        ...
                        //System.out.println("Byte read: " + Integer.valueOf("" + chunk[0]));
                ...
                //System.out.println("Size: " + entries[i].length + "\n");

                f.seek(off_text + start);
                f.read(entries[i]);
            }
            ...
            for (int i = 0; i < num_entries; i++){
                ...
                // Find out if we already have that line in our array
                boolean found = false;
                
                for(int j = 0; j < blines.size() && !found; j++){
                    if (blines.get(j).original.equals(text)){
                        found = true;
                ...
                if (!found){
                    bl = new BattleLine(text, text, "");

                    blines.add(bl);
                    
                    lines_used[i] = blines.size() - 1;
                }
            }
        ...
        try{
            if (use_converted){ // Convert the edited text into letter pairs first
                for (int i = 0; i < blines.size(); i++){
                    blines.get(i).convertLetterPairs();
                }
            }
            
            // Prepare the table and the battle lines' data
            // * The pointer table is contained in the first_section array we stored while parsing the file,
            //  so we overwrite that part (it's always gonna be the same size)
            byte[] off_value;
            
            for (int i = 0; i < lines_used.length; i++){
                byte[] aux;
                
                off_value = int2bytes(size);
                
                first_section[off_table + (i*2)] = off_value[0];
                first_section[off_table + (i*2) + 1] = off_value[1];

                aux = blines.get(lines_used[i]).getBytes(use_converted);
                size += aux.length;

                os.write(aux);
            }
            
            size += first_section.length;
    ...
    // Receives an int and return its 2-byte value
    ...
    public void saveDialogue(){
            blines.get(current_dialogue).edited = textareaEdited.getText();
    }

    
    public void loadDialogue(){
        textareaOriginal.setText(blines.get(current_dialogue).original);
        textareaEdited.setText(blines.get(current_dialogue).edited);
    ...
    public void jumpTo(int num){
        ...
        if (num < 0)
            current_dialogue = 0;
        else if (current_dialogue > (blines.size() - 2) )
            current_dialogue = (blines.size() - 1);
    ...
    public void updateControls(){
        labelCurrent.setText( (current_dialogue + 1) + " / " + blines.size() );
        buttonPrevious.setEnabled(current_dialogue != 0);
        buttonNext.setEnabled(current_dialogue != blines.size() - 1);
    }
    ...
    public void writeWidth(javax.swing.JTextArea ta, javax.swing.JLabel la){
        ...
        la.setText("" + width);
        if (width > 378)    // 44 characters by line, at 9 pixels width
